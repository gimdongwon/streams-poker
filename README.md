...
- **포커 룰 배점** — 스트레이트<플러시<풀하우스<포카드<스트레이트 플러시… 순서로 강함, 투페어는 원페어 2개보다 높음. 족보 표는 점수 내림차순 정렬.
- **총점 최대화 판정** — 조커 배분·스트레이트 위치·페어 묶음을 모두 비교해 **가장 높은 총점**을 내는 조합 집합을 자동 선택

### 게임 모드
- **싱글 플레이** — 혼자 최고 점수에 도전
- **멀티 플레이** — Socket.io 실시간 대전 (최대 10명)
  - 방 만들기(참가비 선택) / 코드로 참여 / **공개 방 목록에서 찾기**(참가비 표시)
  - **라운드 동기화** — 전원이 배치를 완료해야 다음 라운드로 진행
  - **재접속 복구** — 새로고침·일시 끊김 시 진행 중이던 방/보드로 복귀, **라운드 타이머도 서버 기준 남은 시간으로 복원**(풀 리셋 방지)
  - **동점 처리** — 총점이 같으면 타이브레이커(조합에 쓰인 카드 숫자 합)로, 타이브레이커까지 같으면 **공동 순위**

### 코인 & 참가비
- **코인** — 신규 유저 500 코인으로 시작
- **일일 보상** — 하루 1회 **+100 코인** 수령 (KST 기준). 네이티브 앱에선 **리워드 광고(AdMob)** 시청 후 지급
- **참가비 멀티** — 방 생성 시 참가비 선택(**무료 / 100 / 500 / 1000 / 5000 / 10000**)
  - 게임 시작 시 전원 참가비 차감, **상금 = 참가비 × 인원**
  - 종료 시 **1등 몰아주기** (공동 1등이면 균등 분배)
  - 모든 코인 처리는 **서버 권위**(원자적 RPC)로 정산 — 조작 불가

### 랭킹 & 소셜
- **누적 랭킹** — 모든 게임 점수를 합산 (`user_rankings` DB 뷰)
- **리더보드** — 누적 점수 순위 + 각 유저의 **역대 최고 조합** 표시
- **티어 시스템** — 누적 점수로 Bronze → Diamond 5단계, 배지 + **진행도(경험치) 바** (로비 헤더 + 마이페이지)
- **친구** — 아이디로 친구 추가 / 요청 수락·거절 / 친구 목록(티어·누적점수)
- **마이페이지(`/me`)** — 프로필·전적(랭킹·최고 점수·최고 조합)·코인·티어 진행도·친구·설정·계정을 한 곳에 통합
- **닉네임 변경** — 마이페이지에서 언제든 변경 (누적 랭킹은 최신 닉네임 반영)
...
- **공유 썸네일** — Open Graph / Twitter 카드 이미지 (카톡·SNS 링크 미리보기)
...
- **원격 URL 래퍼** — 네이티브 웹뷰가 배포 사이트를 로드, 웹 업데이트가 앱에 즉시 반영
- **친구 요청 푸시 알림** — FCM(HTTP v1)으로 발송, 기본 알림음. (Android 동작, iOS는 APNs 연동 예정)
- **리워드 광고(AdMob)** — 일일 보상에 연결, iOS ATT 동의 처리
- **네이티브 기능** — 카드 배치 햅틱, 네이티브 공유 시트, 가로모드 고정, 스플래시·상태바

### 신뢰성 / 보안
- **서버 권위 점수 계산(anti-cheat)** — 클라이언트가 보낸 점수를 믿지 않고, 서버가 보드를 재평가해 저장
- **점수 계산 회귀 테스트** — `npm test`(node:test + tsx)로 조합/배점 검증 (16케이스)
...
| `0007_coins.sql` | `users.coins`/`last_daily_reward` + 일일보상·차감·지급 원자적 함수 |
| `0008_push_tokens.sql` | 푸시 토큰 저장 테이블(친구요청 알림 발송용) |
...
| `npm test` | 점수 계산 회귀 테스트 (node:test + tsx) |
