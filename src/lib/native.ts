...
// 화면 방향은 네이티브 설정이 양쪽 가로(180° 회전 포함)를 처리한다:
//  - Android: 매니페스트 screenOrientation="sensorLandscape"
//  - iOS: Info.plist UISupportedInterfaceOrientations = LandscapeLeft/Right
// 플러그인 lock('landscape')는 양 플랫폼에서 한쪽 가로로 고정시켜 180° 회전을 막으므로
// 더 이상 호출하지 않는다. (호출부 호환을 위해 no-op 로 유지)
