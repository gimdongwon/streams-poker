...
const Breakdown = ({ combos, total }: { combos: BreakdownRow[]; total: number }) => (
  <div className="mt-2">
    {combos.length > 0 ? (
    ...
    <div className="mt-2 flex items-baseline gap-1.5">
      <span className="text-haze text-[10px] tracking-[2px]">TOTAL</span>
      <span className="text-snow text-base font-extrabold">{total}점</span>
    </div>
  </div>
);
...
const PrimaryButton = ({ label, onClick }: { label: string; onClick: () => void }) => (
  <button
    onClick={onClick}
    className="text-center text-sm font-extrabold text-void py-2.5 rounded-xl active:scale-95 transition-transform"
...
const SecondaryButton = ({ label, onClick }: { label: string; onClick: () => void }) => (
  <button
    onClick={onClick}
    className="text-center text-sm font-bold text-snow py-2.5 rounded-xl border border-edge hover:bg-panel active:scale-95 transition-all"
  >
    {label}
  </button>
);
...
export const ResultScreen = ({
  ...
  const selected = useMemo(() => {
    ...
    return (
      playerResults.find((r) => r.playerId === selectedId) ?? myResult ?? playerResults[0] ?? null
    );
  ...
  const boardTotal =
    mode === "single" ? totalScore : (selected?.score ?? 0);
  ...
  return (
    <div className="h-[100dvh] w-full flex flex-col p-3 overflow-hidden">
      ...
      <div className="flex-1 min-h-0 flex flex-col landscape:flex-row gap-3 overflow-y-auto landscape:overflow-hidden">
        {/* LEFT — verdict + actions */}
        <motion.div
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
          className="landscape:w-[28%] flex flex-col shrink-0"
        >
          {mode === "single" ? (
            <>
              ...
              <div className="flex items-baseline gap-1 mt-1">
                <span
                  className="text-[3.4rem] leading-none font-extrabold"
                  style={{
                ...
                <span className="text-haze text-lg font-bold">점</span>
              </div>
              {playerRank != null && (
                <div className="mt-2 text-xs text-snow">
                  전체 <span className="text-neon-cyan font-extrabold">{playerRank}위</span>
                </div>
              )}
              {combinations.length > 0 && (
                <div className="mt-1 text-[11px] text-haze">
                  BEST · {combinations[0].name}
                </div>
              )}
              <div className="mt-2 text-[11px] h-4">
                {saveStatus === "saving" && <span className="text-haze">저장 중...</span>}
                {saveStatus === "saved" && <span className="text-neon-cyan">기록 저장됨</span>}
                {saveStatus === "error" && <span className="text-red-400">저장 실패</span>}
              </div>
            </>
          ) : (
            <>
              <span className="text-haze text-[10px] tracking-[2px]">YOU PLACED</span>
              <div className="text-[3rem] leading-none font-extrabold text-neon-cyan mt-1">
                {myResult ? ordinal(myResult.rank) : "-"}
              </div>
              {myResult && (
                <div className="mt-1 text-xs text-snow">
        ...
        <motion.div
          ...
          className="landscape:flex-1 min-w-0 flex flex-col landscape:border-x landscape:border-edge/60 landscape:px-3"
        >
          <div className="flex items-center justify-between mb-1">
            <span className="text-haze text-[10px] tracking-[2px]">
              {mode === "multi" && selected
                ? `${selected.nickname}의 보드`
                : "MY BOARD"}
            </span>
            {mode === "multi" && (
          ...
          <Breakdown combos={breakdownRows} total={boardTotal} />
        </motion.div>
        ...
        {mode === "multi" && (
          <motion.div
            ...
            className="landscape:w-[29%] flex flex-col shrink-0"
          >
            ...
            <div className="flex-1" />
            <p className="text-haze/70 text-[9px] text-center pt-2">
